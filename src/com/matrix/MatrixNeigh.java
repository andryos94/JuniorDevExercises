...
public class MatrixNeigh {
	...
	public static void main(String[] args) {
		int n = random.nextInt(dimensionLimit - 2) + 2, m = random.nextInt(dimensionLimit - 2) + 2;
		...
		System.out.println("The (randomly) chosen point is: matrix[" + elementI + "][" + elementJ + "]="
				+ mat[elementI][elementJ]);
		System.out.println();
		...
		for (int i = 0; i < dirX.length; i++) {
			...
			if (neighI >= 0 && neighI < n && neighJ >= 0 && neighJ < m) {
				System.out.println("The " + dirName[i] + " neighbor is: matrix[" + neighI + "][" + neighJ + "]="
						+ mat[neighI][neighJ]);
				countNeigh++;
			}
		}
